...
int main(int, char *argv[])
{
  ...
  try
  {
    ...
    */
    ...
    // belief update
    // CLUSTER 1
    rcptr<Factor> sep1new = clusternode2->marginalize({b0, b2, b3});
    clusternode1 = sep1new->cancel(sep1)->absorb(clusternode1);
    sep1 = sep1new;

    rcptr<Factor> sep2new = clusternode3->marginalize({b0, b1, b3});
    clusternode1 = sep2new->cancel(sep2)->absorb(clusternode1);
    sep2 = sep2new;

    rcptr<Factor> sep3new = clusternode4->marginalize({b0, b1, b2});
    clusternode1 = sep3new->cancel(sep3)->absorb(clusternode1);
    sep3 = sep3new;

    // cluster 2
    sep1new = clusternode1->marginalize({b0, b2, b3});
    clusternode2 = sep1new->cancel(sep1)->absorb(clusternode2);
    sep1 = sep1new;

    //cluster 3
    sep2new = clusternode1->marginalize({b0, b1, b3});
    clusternode3 = sep2new->cancel(sep2)->absorb(clusternode3);
    sep2 = sep2new;

    sep3new = clusternode1->marginalize({b0, b1, b2});
    clusternode4 = sep3new->cancel(sep3)->absorb(clusternode4);
