...
int main(int, char *argv[])
{
  ...
  try
  {
    ...
    */
    ...
    rcptr<Factor> clusternode4 = b4given->absorb(r0given->observeAndReduce({r0}, {1}))->absorb(r1given->observeAndReduce({r1}, {0}))->
    ...
    rcptr<Factor> ogclusternode1 = clusternode1;
    
    // one cycle of belief propagation-- we get convergence 
    m41 = clusternode4->marginalize({b0, b1, b2});
    m31 = clusternode3->marginalize({b0, b1, b3});
    m21 = clusternode2->marginalize({b0, b2, b3});
    m12 = m41->absorb(clusternode1)->absorb(m31)->marginalize({b0, b2, b3});
    m13 = m21->absorb(clusternode1)->absorb(m41)->marginalize({b0, b1, b3});
    m14 = m21->absorb(clusternode1)->absorb(m31)->marginalize({b0, b1, b2});

    clusternode1 = clusternode1->absorb(m21)->absorb(m31)->absorb(m41)->normalize();
    clusternode2 = clusternode2->absorb(m12)->normalize();
    clusternode3 = clusternode3->absorb(m13)->normalize();
    clusternode4 = clusternode4->absorb(m14)->normalize();
    ...
    std::cout << *clusternode1->marginalize({b3})->normalize() << std::endl;
    std::cout << *clusternode4->marginalize({b4})->normalize() << std::endl;
